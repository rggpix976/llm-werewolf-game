...
- The Phase 6 NPC reaction retry-policy maximum is now defined as a new pre-runtime architecture decision: `maxAttempts` is a safe integer in `1..8`, while `3` remains only the initial default. Each logical reaction captures an immutable engine-owned snapshot at creation; AI/provider output and later global-default changes cannot alter it. The bound limits one logical reaction's provider-invocation exposure and keeps attempt/control metadata finite. It is independent of the delivery controller's separate three-sink-attempt policy. Runtime validators and coordinator behavior are unchanged; the next docs step is the Tombstone Retry-Policy Evidence Contract, which will add historical evidence without changing the seven-field root.
...
1. Review and merge the docs-only retry-policy maximum decision without changing runtime or the current tombstone schema.
2. After that review, define the separate Tombstone Retry-Policy Evidence Contract using the fixed `1..8` protocol range, then resume the separately scoped coordinator Goal only after that contract is merged.
