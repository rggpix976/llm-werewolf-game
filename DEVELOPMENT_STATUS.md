...
- The successful-attempt reference contract is now defined without runtime changes. `NpcReactionPlan.successfulAttemptId` references the engine-owned winning `reactionAttemptId`; the plan, preparation binding, commit delta, and commit result strict schemas remain unchanged. Schema-only, pre-commit, and post-commit validation are separate: pre-commit resulting version is owned by the commit delta, while post-commit successful-attempt authority is the plan plus NPC idempotency record and post-commit `turnOrder` is owned only by that idempotency record. Complete committed-graph validation remains coordinator/tombstone independent across cleanup and replay. Runtime Contract Alignment remains stopped until this contract is merged; preparation, commit, replay, coordinator, tombstone, and delivery runtime remain unimplemented.
...
- Result: 321/321 tests passed. The focused Phase 6 candidate-validation suite passed 21/21 and the Phase 6 foundation suite passed 9/9. `npm.cmd run sample`, `git diff --check`, all 31 documentation JSON blocks, all 7 JavaScript example blocks, successful-attempt ownership/context/version-chain checks, idempotency-only post-commit turn-order checks, cleanup-independence and active-rejection-set markers, UTF-8/BOM, privacy/secret, conflict-marker, and forbidden-Unicode scans passed. No dependency, package, lockfile, game-engine, browser, CLI, server, provider, endpoint, test, or authoritative-state route changed.
...
1. After the successful-attempt reference contract is merged, resume the separately reviewed Runtime Contract Alignment task without extending the plan, preparation binding, or commit result schemas.
2. Then re-audit and split pure preparation, non-routing commit, provider/delivery harnesses, coordinator/route integration, and default-off verification into separately reviewed implementation stages.
3. Keep the legacy provisional NPC transaction as the active runtime route until those stages are separately implemented and reviewed. The old Phase 6 implementation Goal remains BLOCKED history.
...
- The authoritative Phase 6 docs define candidate validation, canonical-only reaction preparation, authoritative commit, shared publication-ledger ownership, the complete session-local coordinator control root, and the independent canonical NPC delivery lifecycle without changing Phase 4/5 runtime behavior. Validation-only implementation is merged but remains production-unconnected; preparation/commit/replay/coordinator/tombstone runtime, plan/candidate validator alignment, structured route replacement, provider integration, and delivery remain unimplemented. The exact Phase 7 finalization append API and controlled Renderer implementation remain pending separate work.
