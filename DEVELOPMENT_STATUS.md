...
- The Phase 6 NPC candidate-validation contract is defined authoritatively, including exact raw success-response transport evidence, the pure validation input, observed-candidate fingerprint ownership, live applicability snapshot, all pending/logical status routes, terminal duplicate/conflict versus hard-stale precedence, total evaluation order, transport reason-code ownership, request/response envelopes, proposal authorization, immutable validated value, redacted rejection union, and validation-only lifecycle boundary. Header shape/semantic ownership, every active rejection code's provider-reachable vector, reserved non-rejection identifiers, and exact `result_claim` first-failure precedence are also closed. A validation-only implementation exists only on an unmerged Draft branch; it must be synchronized with the latest `master`, corrected to this contract, and reviewed again. The previously blocked Phase 6 implementation Goal remains BLOCKED and has not been resumed or changed.
- The isolated validation-only implementation now exposes one synchronous `validateNpcReactionCandidate(input)` entrypoint plus closed constants and `NpcReactionCandidateValidationInvariantError`. It implements exact stages 0 through 18, raw transport validation, strict detached candidate reconstruction, engine-owned fingerprints, authorization, stale/duplicate/conflict classification, and recursively frozen results. It is not imported by `WerewolfGame`, browser, CLI, server, provider, or HTTP adapters and performs no provider call, route change, state mutation, version increment, publication, or display.
...
1. After this docs-only correction is reviewed and merged, synchronize the separate Draft validation-only implementation with that latest `master`, apply the closed rejection/header/result-claim contract, and submit it for renewed review under an explicitly approved implementation Goal.
...
- The authoritative Phase 6 docs define candidate validation without changing Phase 4/5 runtime behavior. Validation-only implementation remains unmerged; production routing and later provider/coordinator/commit/publication work remain unimplemented.
