...
- C1 also defines one nonauthoritative session-local `NpcReactionCoordinatorControlRoot` containing logical reactions, attempts, terminal reservations, tombstones, and terminal order. Complete planned-logical creation atomically inserts its logical entry and reservation, consumes one terminal order only on root replacement, and checks an engine-built authoritative committed-plan collision projection. Attempt creation and terminalization are separate one-root transactions; cleanup failure retains the complete committed cleanup-pending old root; reset destroys the full root and runtime handles. Combined reservation/tombstone capacity is 1024, eviction of the unique oldest tombstone occurs only with successful complete creation, and capacity/order/identity/projection failures leave no orphan, eviction, or counter gap. This remains design only. Exact NPC browser/CLI delivery ownership is intentionally deferred to Docs PR C2, and the legacy provisional NPC route remains the current runtime.
...
- Result: 321/321 tests passed. The focused Phase 6 candidate-validation suite passed 21/21 and the Phase 6 foundation suite passed 9/9. `npm.cmd run sample`, `git diff --check`, all 29 documentation JSON blocks, code-fence/signature checks, authority/version classification, exact seven-field coordinator-root ownership, planned creation/terminalization/cleanup/reset vectors, UTF-8/BOM, privacy/secret, conflict-marker, and forbidden-Unicode scans passed. No dependency, package, lockfile, game-engine, browser, CLI, server, provider, endpoint, or authoritative-state route changed.
...
- The authoritative Phase 6 docs define candidate validation, canonical-only reaction preparation, authoritative commit, shared publication-ledger ownership, and the complete session-local coordinator control root without changing Phase 4/5 runtime behavior. Validation-only implementation is merged but remains production-unconnected; preparation/commit/replay/coordinator/tombstone runtime, plan/candidate validator alignment, structured route replacement, provider integration, and delivery remain unimplemented. Exact Phase 7 finalization append API and exact NPC delivery remain pending separate docs work, including Docs PR C2.
