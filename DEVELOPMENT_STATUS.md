...
- Docs PR C1 closes runtime ownership and ledger semantics without adding runtime: the existing `state.conversation.publications`, `nextPublicationSlotOrder`, and `nextRecordAppendOrder` fields form one authoritative registry shared by player, NPC, and future Phase 7 publication records, with dense integrity, exhaustion, replay, rollback, and delivery separation. No alternate canonical-counter field, alias, mirror, NPC-only ledger, or per-record-type counter is permitted.
- C1 also defines the nonauthoritative session-local `NpcReactionCoordinatorState`: terminal order starts at zero and advances only with a successful reservation; reservation-to-tombstone conversion preserves the order; combined capacity is 1024; only the minimum-order tombstone may be evicted; all-reservation capacity and safe-integer exhaustion fail before ID allocation with no gap; reset destroys the complete coordinator domain. This remains design only. Exact NPC browser/CLI delivery ownership is intentionally deferred to Docs PR C2, and the legacy provisional NPC route remains the current runtime.
...
- Result: 321/321 tests passed. The focused Phase 6 candidate-validation suite passed 21/21 and the Phase 6 foundation suite passed 9/9. `npm.cmd run sample`, `git diff --check`, documentation JSON/signature checks, commit stages 0 through 23, replay-before-reservation, existing shared publication-counter authority, dense ledger arithmetic, terminal-order/capacity/eviction/reset vectors, terminal tombstone subset, lifecycle 7-by-8, browser import-boundary, no-routing, privacy/secret, conflict-marker, and forbidden-Unicode scans passed. No dependency, package, lockfile, game-engine, browser, CLI, server, provider, endpoint, or authoritative-state route changed.
...
1. Prepare Docs PR C2 to close the exact committed NPC publication discovery, browser/CLI sink evidence, retry/acknowledgement, replay/history, reset, and feature-mode contract. Do not infer an NPC API from the player protocol.
2. After C2 is merged, re-audit and split validator alignment, pure preparation, non-routing commit, provider/delivery harnesses, coordinator/route integration, and default-off verification into separately reviewed implementation stages. The legacy provisional NPC transaction remains the active runtime route, and the old Phase 6 implementation Goal remains BLOCKED history.
...
- The authoritative Phase 6 docs define candidate validation, canonical-only reaction preparation, authoritative commit, the existing shared publication-ledger ownership, and the session-local terminal-order ledger without changing Phase 4/5 runtime behavior. Validation-only implementation is merged but remains production-unconnected; preparation/commit/replay/coordinator/tombstone runtime, plan/candidate validator alignment, structured route replacement, provider integration, and delivery remain unimplemented. Exact NPC delivery remains pending Docs PR C2.
