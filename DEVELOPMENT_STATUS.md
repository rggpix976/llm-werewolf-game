...
- Finding F-02／F-03 hardeningはbranch `fix/phase6-player-first-npc-delivery`上のDraft変更として実装中である。Structured Route／authoritative NPC Commitとlive Deliveryを分離し、engine-private handoffはPlayer publicationが`acknowledged`または`evidence_recorded`になるまでNPC pumpを開始しない。Browser／CLIはPlayer writerをawaitした後だけ明示completionを呼ぶ。Browser New Gameは旧generationを先にinvalidateし、旧session NPC DOM nodeをbookkeeping reset前に除去し、stale sink callbackとuntracked node再挿入を拒否する。Authority、Provider、Validation、Preparation、Commit、Delivery controller／orchestrator／sink contractは変更せず、`NPC_STRUCTURED_REACTION_MODE`はdefault-off、API key操作とbillable live smokeは0である。Independent reviewとmergeは未完了である。
...
- Result: 651/651 local tests and the required 158/158 focused tests pass on the Finding F-02／F-03 hardening branch, including the deterministic Player-display gate, duplicate/concurrent completion, display failure/retry, destroy invalidation, actual Browser entrypoint New Game cleanup, Browser generation isolation, and Browser／CLI sequencing coverage. Existing F-01 cost controls, Slice 6 Route/Commit/Delivery, Phase 5 Player delivery, Coordinator, Provider, and sink matrices remain green. `npm.cmd run sample`, changed-module syntax checks, browser-safe import scans, and `git diff --check` pass against the working Draft branch. Independent review and merge remain pending; no merge-commit workflow run is claimed at this branch state. No dependency, package, lockfile, or workflow changed. No API key was created, read, displayed, or used and no billable OpenAI smoke was run.
...
1. Independently review and merge the Draft F-02 Player-before-NPC display sequencing and F-03 Browser New Game cleanup hardening while keeping `NPC_STRUCTURED_REACTION_MODE` default-off.
2. F-04 observability, F-05 rollback runbook, and F-06 acceptance coverage remain separate unresolved hardening items. This change does not implement them and does not make a flag-on or production-readiness claim. Persistence/cross-process recovery, authentication, distributed rate limiting, and Phase 7/8 remain out of scope.
...
- Branch `fix/phase6-player-first-npc-delivery` implements Findings F-02／F-03 from authoritative baseline `4a510d902c7d455d7ed7c3c1e96a5d89c814f9ea` as an unmerged Draft change. F-01 is already merged. The branch changes only the production sequencing handoff, Browser/CLI entrypoint ordering, Browser session DOM cleanup, focused tests, and status documents; F-04〜F-06 remain unresolved.
