...
export function buildShadowInterpreterRequest({ snapshot, rawText, gameId, requestId, correlationId }) {
  ...
  const request = { schemaVersion: 1, requestId, correlationId, turnId: `shadow-turn-${gameId}-${snapshot.day}`, preconditionStateVersion: 0, preconditionPhase: snapshot.phase, locale: "ja-JP", rawText, playerContext: { playerId: "player", publicStatus: "alive" }, publicRoster: roster, allowedCandidateTypes: [...candidateTypes], publicContext: { publicEvents: [], publicClaims: [] }, limits: { maxAlternatives: 3, maxActsPerAlternative: 4, maxNestingDepth: 8 } };
...
export class InterpreterShadowClient {
  ...
  observe({ snapshot, rawText, gameId, targetNpcId }) {
    const request = buildShadowInterpreterRequest({ snapshot, rawText, gameId, requestId: `interpreter-${this.createId()}`, correlationId: `correlation-${this.createId()}` }), started = this.now();
    void this.provider.interpretPlayerInput(request, { targetNpcId }).then((response) => {
      if (!this.sessionManager.isCurrentGame(gameId)) return;
      ...
      if (!this.sessionManager.isCurrentGame(gameId) || error?.name === "AbortError") return;
