...
function safeInteger(value, min, path, max = Number.MAX_SAFE_INTEGER) { if (!Number.isSafeInteger(value) || value < min || value > max) fail(path, `must be a safe integer from ${min} to ${max}`); }
...
export function validateNpcReactionPlan(value) {
  const path = "reactionPlan", common = ["schemaVersion", "requestId", "correlationId", "causationId", "originatingInputRecordId", "locale", "causationEventIds", "reactionPlanId", "successfulAttemptId", "turnId", "preconditionStateVersion", "resultingStateVersion", "npcId", "renderMode", "intendedSpeechActs", "policies", "maxChars"];
  ...
  id(value.successfulAttemptId, `${path}.successfulAttemptId`); safeInteger(value.preconditionStateVersion, 0, `${path}.preconditionStateVersion`, Number.MAX_SAFE_INTEGER - 1); safeInteger(value.resultingStateVersion, 1, `${path}.resultingStateVersion`); if (value.resultingStateVersion !== value.preconditionStateVersion + 1) fail(`${path}.resultingStateVersion`, "must equal preconditionStateVersion plus one", "state_version_mismatch");
  ...
  const expectedPolicies = value.renderMode === "controlled_commentary" ? { allowStateChanges: false, allowClaims: false, allowVoteDeclaration: false, allowSuspicionUpdate: false, allowMemoryUpdate: false } : { allowStateChanges: true, allowClaims: types.has("role_claim") || types.has("result_claim"), allowVoteDeclaration: types.has("vote_declaration"), allowSuspicionUpdate: types.has("suspicion"), allowMemoryUpdate: false };
