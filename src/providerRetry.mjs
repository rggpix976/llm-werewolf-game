...
export async function runProviderWithRetry(operation, options = {}) {
  const policy = { ...defaults, ...(options.policy ?? {}) }, signal = options.signal, now = options.now ?? Date.now, setTimer = options.setTimeout ?? globalThis.setTimeout, clearTimer = options.clearTimeout ?? globalThis.clearTimeout, delay = options.delay ?? ((ms, delaySignal) => abortableDelay(ms, delaySignal, { setTimeout: setTimer, clearTimeout: clearTimer })), started = now(); let lastError;
  for (let attempt = 1; attempt <= policy.maxAttempts; attempt += 1) {
    ...
    const controller = new AbortController(), timeoutMs = Math.min(policy.perAttemptTimeoutMs, remaining - policy.responseValidationBudgetMs), timer = setTimer(() => controller.abort(abortError("Provider attempt timed out")), timeoutMs), onAbort = () => controller.abort(signal.reason);
