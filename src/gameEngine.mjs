...
export class WerewolfGame {
  ...
  constructor(state, responseProvider = new PseudoResponseProvider(), options = {}) {
    ...
    this.playerPublicationDeliveryController = new PlayerPublicationDeliveryController({ gameSessionId: state.gameSessionId, createId: this.createId, observer: this.playerStructuredConsumerObserver, enabled: this.playerStructuredConsumerEnabled, initialWatermark: 0, listPublications: () => this.state.conversation.publications.filter((record) => record.recordType === "player_utterance_published").sort((a, b) => a.publicationSlotOrder - b.publicationSlotOrder), resolvePublication: (publicationId, deliveryMode) => deliveryMode === "legacy" ? this._legacyPlayerPublication(publicationId) : this._displayPlayerPublication(this._renderPlayerPublication(publicationId)) });
  ...
  _actionResult(actionType, result, logCursor) {
    ...
    if (actionType !== "get_state" && result?.replayed !== true) { const deliveryMode = this.playerStructuredConsumerEnabled ? "structured" : "legacy", candidates = this.playerPublicationDeliveryController.discoverCandidates(deliveryMode); deliveryPublicationIds = candidates.map((candidate) => candidate.publicationId); if (deliveryMode === "structured") structuredPlayerEntries = this._renderPlayerPublicationIds(candidates.filter((candidate) => !candidate.acknowledgementOnly).map((candidate) => candidate.publicationId)).map((entry) => this._displayPlayerPublication(entry)); livePlayerDisplayEntries = this._livePlayerDisplayEntries(playerLogEntries, logCursor, candidates, deliveryMode); }
  ...
  _syncStructuredConsumerMode() {
    ...
    this.playerPublicationDeliveryController.setEnabled(this.playerStructuredConsumerEnabled, this.state.conversation.nextPublicationSlotOrder); this._consumerModeObserved = this.playerStructuredConsumerEnabled;
  }
  ...
  _livePlayerDisplayEntries(entries, startOrder, candidates, deliveryMode) { const mappings = new Map(this.state.conversation.playerLegacyDisplayCompatibilityRecords.map((mapping) => [mapping.legacyLogAppendOrder, mapping.publicationId])), mappingByPublication = new Map(this.state.conversation.playerLegacyDisplayCompatibilityRecords.map((mapping) => [mapping.publicationId, mapping])), liveScope = this.playerPublicationDeliveryController.liveScopePublicationIds(), preCutoverDelivered = this.playerPublicationDeliveryController.preCutoverLegacyDeliveredPublicationIds(), preCutoverPending = new Set(this.playerPublicationDeliveryController.pendingPreCutoverLegacyPublicationIds()), pending = new Set(candidates.map((candidate) => candidate.publicationId)), ordered = [...candidates.map((candidate) => ({ order: mappingByPublication.get(candidate.publicationId).legacyLogAppendOrder, envelope: Object.freeze({ kind: "player_publication_delivery", ...candidate }) })), ...[...preCutoverPending].map((publicationId) => { const mapping = mappingByPublication.get(publicationId); return { order: mapping.legacyLogAppendOrder, envelope: Object.freeze({ kind: "legacy_display", entry: structuredClone(this.state.playerLog[mapping.legacyLogAppendOrder]), preCutoverPublicationId: publicationId }) }; })];
    for (let index = 0; index < entries.length; index += 1) { const order = startOrder + index, publicationId = mappings.get(order); if (!publicationId) ordered.push({ order, envelope: Object.freeze({ kind: "legacy_display", entry: structuredClone(entries[index]) }) }); else if (deliveryMode === "legacy" && !liveScope.has(publicationId) && !preCutoverDelivered.has(publicationId) && !preCutoverPending.has(publicationId) && !pending.has(publicationId)) ordered.push({ order, envelope: Object.freeze({ kind: "legacy_display", entry: structuredClone(entries[index]) }) }); }
  ...
  recordPreCutoverLegacyDelivery(publicationId) { return this.playerPublicationDeliveryController.recordPreCutoverLegacyDelivery(publicationId); }
