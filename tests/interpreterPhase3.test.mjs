...
test("vote and night remain one transaction each and authoritative lifecycle is flag-independent", async () => {
  ...
  assert.deepEqual(getRuntimeConfig(parseConfig({ INTERPRETER_SHADOW_MODE: "true", INTERPRETER_VALIDATION_MODE: "true" })), { provider: "pseudo", interpreterShadowMode: true, interpreterValidationMode: true, playerConversationCommitMode: false, playerStructuredConsumerMode: false, npcStructuredReactionMode: false });
});
