...
import { mergeStructuredPlayerEntries, renderPlayerPublication, renderUnconsumedPlayerPublications, resolvePlayerStructuredConsumerPolicy, sanitizeTerminalText } from "../src/playerStructuredConsumer.mjs";
...
function render(instance, publicationId = instance.state.conversation.publications[0].publicationId) { return renderPlayerPublication({ gameSessionId: instance.state.gameSessionId, conversation: instance.state.conversation, publicationId, publicParticipantsById: participants(instance) }); }
...
test("dangling, duplicate, and invalid publication graphs fail closed with typed consumer errors", async () => {
  ...
  const missingInput = structuredClone(instance.state.conversation); missingInput.inputRecords = []; assert.throws(() => renderPlayerPublication({ gameSessionId: instance.state.gameSessionId, conversation: missingInput, publicationId, publicParticipantsById: participants(instance) }), (error) => error.code === "dangling_input_reference");
  const duplicate = structuredClone(instance.state.conversation); duplicate.publications.push(structuredClone(duplicate.publications[0])); assert.throws(() => renderPlayerPublication({ gameSessionId: instance.state.gameSessionId, conversation: duplicate, publicationId, publicParticipantsById: participants(instance) }), (error) => error.code === "duplicate_publication");
  const missingParticipant = participants(instance); delete missingParticipant.npc1; const claimInstance = await committed([{ type: "result_claim", targetId: "npc1", result: "werewolf", sourceSpan: { start: 0, end: 5 } }]); assert.throws(() => renderPlayerPublication({ gameSessionId: claimInstance.state.gameSessionId, conversation: claimInstance.state.conversation, publicationId: claimInstance.state.conversation.publications[0].publicationId, publicParticipantsById: missingParticipant }), (error) => error.code === "missing_public_participant_projection");
  assert.throws(() => renderPlayerPublication({ gameSessionId: instance.state.gameSessionId, activeGameSessionId: "game-new", conversation: instance.state.conversation, publicationId, publicParticipantsById: participants(instance) }), (error) => error.code === "mismatched_session");
});

test("Phase 5 cutover emits one structured entry, suppresses legacy display, and preserves NPC order", async () => {
  let observed; const instance = game([{ type: "non_game_statement", sourceSpan: { start: 0, end: 5 } }], true); instance.playerStructuredConsumerObserver = (entry) => { observed = entry; }; const action = await instance.dispatchPlayerAction({ type: "ask_npc", targetId: "npc1", input: "hello" });
  assert.equal(action.structuredPlayerEntries.length, 1); assert.equal(action.playerFacingEntries.length, 2); assert.equal(action.playerFacingEntries[0].structured, true); assert.equal(action.playerFacingEntries[0].message, "hello"); assert.equal(action.playerFacingEntries[1].message, "Aoi: NPC response"); assert.equal(action.playerFacingEntries.some((entry) => entry.message.includes("あなた ->")), false);
  assert.deepEqual(Object.keys(observed), ["gameSessionId", "correlationId", "publicationId", "displayPlanId", "outcomeCategory", "reasonCode", "consumerMode", "duplicateSuppressed"]); assert.equal(JSON.stringify(observed).includes("hello"), false);
});

test("exact replay emits no display/history entry and never rewinds the consumer cursor", async () => {
  const instance = game([{ type: "non_game_statement", sourceSpan: { start: 0, end: 5 } }], true); await instance.dispatchPlayerAction({ type: "ask_npc", targetId: "npc1", input: "hello" }); const record = instance.state.conversation.idempotencyRecords[0], consumed = new Set(instance._consumedPlayerPublicationIds), replay = await instance.dispatchPlayerAction({ type: "ask_npc", replayRequestId: record.requestId, replayRequestFingerprint: record.requestFingerprint }); assert.deepEqual(replay.structuredPlayerEntries, []); assert.deepEqual(replay.playerFacingEntries, []); assert.deepEqual(instance._consumedPlayerPublicationIds, consumed);
});
...
test("slot ordering is authoritative while append order and legacy array position are ignored", async () => {
  const first = await committed([{ type: "non_game_statement", sourceSpan: { start: 0, end: 5 } }]), entries = renderUnconsumedPlayerPublications({ gameSessionId: first.state.gameSessionId, conversation: first.state.conversation, publicParticipantsById: participants(first), consumedPublicationIds: new Set() }); assert.equal(entries.length, 1); const merged = mergeStructuredPlayerEntries([{ day: 1, phase: "player_question", message: "legacy" }, { day: 1, phase: "npc_response", message: "npc" }], entries); assert.deepEqual(merged.map((entry) => entry.message), ["hello", "npc"]);
});
...
test("browser and CLI adapters select derived entries and retain safe output boundaries", () => {
  ...
  assert.match(cli, /action\.playerFacingEntries/); assert.match(cli, /sanitizeTerminalText/);
});
