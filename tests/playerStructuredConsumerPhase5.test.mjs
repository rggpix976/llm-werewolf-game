...
import { deliverProjectedEntries } from "../src/playerDisplaySink.mjs";
...
test("Phase 5 cutover emits one structured entry, suppresses legacy display, and preserves NPC order", async () => {
  ...
  assert.equal(action.structuredPlayerEntries.length, 1); assert.equal(action.playerFacingEntries.length, 2); await deliverProjectedEntries({ game: instance, entries: action.playerFacingEntries, consumerId: "test", sinkType: "browser", writeStructured: async (entry) => visible.push(entry), writeLegacy: async (entry) => visible.push(entry) }); assert.equal(visible[0].structured, true); assert.equal(visible[0].message, "hello"); assert.equal(visible[1].message, "Aoi: NPC response"); assert.equal(visible.some((entry) => entry.message.includes("あなた ->")), false); assert.equal(observed.filter((entry) => entry.outcomeCategory === "publication_acknowledged").length, 1);
});

test("exact replay emits no display/history entry and never rewinds the consumer cursor", async () => {
  const instance = game([{ type: "non_game_statement", sourceSpan: { start: 0, end: 5 } }], true), first = await instance.dispatchPlayerAction({ type: "ask_npc", targetId: "npc1", input: "hello" }); await deliverProjectedEntries({ game: instance, entries: first.playerFacingEntries, consumerId: "test", sinkType: "cli", writeStructured: async () => {}, writeLegacy: async () => {} }); const record = instance.state.conversation.idempotencyRecords[0], version = instance.state.stateVersion, replay = await instance.dispatchPlayerAction({ type: "ask_npc", replayRequestId: record.requestId, replayRequestFingerprint: record.requestFingerprint }); assert.deepEqual(replay.structuredPlayerEntries, []); assert.deepEqual(replay.playerFacingEntries, []); assert.equal(instance.state.stateVersion, version);
});
