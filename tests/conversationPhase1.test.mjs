...
function npcReferenceFixtures() {
  const plan = canonicalPlan(), binding = { schemaVersion: 1, gameSessionId: "session-1", reactionPlanId: plan.reactionPlanId, successfulAttemptId: plan.successfulAttemptId, requestId: plan.requestId, requestFingerprint: fingerprint, correlationId: plan.correlationId, causationId: plan.causationId, originatingInputRecordId: plan.originatingInputRecordId, turnId: plan.turnId, turnOrder: 1, preconditionPhase: "player_question", preconditionStateVersion: plan.preconditionStateVersion, npcId: plan.npcId }, validatedCandidateBinding = { gameSessionId: binding.gameSessionId, reactionPlanId: binding.reactionPlanId, reactionAttemptId: binding.successfulAttemptId, requestId: binding.requestId, requestFingerprint: binding.requestFingerprint, correlationId: binding.correlationId, causationId: binding.causationId, originatingInputRecordId: binding.originatingInputRecordId, turnId: binding.turnId, turnOrder: binding.turnOrder, preconditionPhase: binding.preconditionPhase, preconditionStateVersion: binding.preconditionStateVersion, npcId: binding.npcId };
  const source = { sourceType: "npc_reaction", reactionPlanId: plan.reactionPlanId, descriptorId: "desc-1", originatingInputRecordId: plan.originatingInputRecordId, reactionCommitRequestId: plan.requestId }, npcClaim = { ...claim, claimId: "claim-2", actorId: plan.npcId, source, createdStateVersion: plan.resultingStateVersion, idempotencyKey: npcClaimIdempotencyKey({ reactionCommitRequestId: plan.requestId, reactionPlanId: plan.reactionPlanId, descriptorId: "desc-1", actorId: plan.npcId, claimKind: "role_claim" }) }, npcEvent = { ...eventBase, eventId: "npc-event-1", requestId: plan.requestId, turnId: plan.turnId, actorId: plan.npcId, causationId: plan.causationId, correlationId: plan.correlationId, source, stateVersion: plan.resultingStateVersion, occurredPhase: "player_question", createdOrder: 2, eventType: "role_claim_recorded", claimId: npcClaim.claimId }, publication = { schemaVersion: 1, recordType: "npc_canonical_published", publicationId: "npc-publication-1", reactionPlanId: plan.reactionPlanId, reactionCommitRequestId: plan.requestId, originatingInputRecordId: plan.originatingInputRecordId, correlationId: plan.correlationId, turnId: plan.turnId, reactionResultingStateVersion: plan.resultingStateVersion, actorId: plan.npcId, locale: plan.locale, canonicalRendererVersion: 1, canonicalSegmentIds: ["seg-1"], publicationSlotOrder: 1, recordAppendOrder: 1 }, commitResult = { schemaVersion: 1, requestId: plan.requestId, correlationId: plan.correlationId, requestFingerprint: fingerprint, commitType: "npc_reaction", preconditionStateVersion: plan.preconditionStateVersion, resultingStateVersion: plan.resultingStateVersion, reactionPlanId: plan.reactionPlanId, npcPublicationId: publication.publicationId, createdEventIds: [npcEvent.eventId], createdClaimIds: [npcClaim.claimId], createdAtOrder: 3, resultMode: "canonical_only" };
  const commitDelta = { schemaVersion: 1, commitType: "npc_reaction", resultMode: "canonical_only", binding: structuredClone(binding), preparationFingerprint: "a".repeat(64), requestFingerprint: fingerprint, candidateFingerprint: "b".repeat(64), projectionFingerprint: "c".repeat(64), preconditionPhase: "player_question", resultingPhase: "player_question", preconditionStateVersion: plan.preconditionStateVersion, resultingStateVersion: plan.resultingStateVersion, plan: structuredClone(plan), claims: [structuredClone(npcClaim)], events: [structuredClone(npcEvent)], publication: structuredClone(publication), effects: { suspicionScoreUpdates: [], memoryUpdates: [], legacyPublicHistoryEntries: [], voteStateUpdates: [], phaseTransitions: [] }, artifactAllocation: { schemaVersion: 1, allocationType: "npc_reaction_artifacts", descriptorIds: ["desc-1"], claimAllocations: [{ proposalIndex: 0, claimId: npcClaim.claimId }], eventIds: [npcEvent.eventId], segmentIds: ["seg-1"], publicationId: publication.publicationId }, orderReservation: { schemaVersion: 1, reservationType: "npc_reaction_orders", preconditionNextCreatedOrder: 2, eventCreatedOrders: [2], commitResultCreatedAtOrder: 3, resultingNextCreatedOrder: 4, preconditionNextPublicationSlotOrder: 1, publicationSlotOrder: 1, resultingNextPublicationSlotOrder: 2, preconditionNextRecordAppendOrder: 1, publicationRecordAppendOrder: 1, resultingNextRecordAppendOrder: 2, priorClaimCount: 1, priorEventCount: 1 }, expectedCommitResult: structuredClone(commitResult), idempotencyReservation: { schemaVersion: 1, requestId: plan.requestId, requestFingerprint: fingerprint, reactionPlanId: plan.reactionPlanId, successfulAttemptId: plan.successfulAttemptId, preparationFingerprint: "a".repeat(64) } };
  const idempotencyRecord = { schemaVersion: 1, recordType: "npc_reaction_commit_idempotency", gameSessionId: binding.gameSessionId, reactionPlanId: plan.reactionPlanId, requestId: plan.requestId, requestFingerprint: fingerprint, preparationFingerprint: "a".repeat(64), successfulAttemptId: plan.successfulAttemptId, correlationId: plan.correlationId, causationId: plan.causationId, originatingInputRecordId: plan.originatingInputRecordId, turnId: plan.turnId, turnOrder: binding.turnOrder, npcId: plan.npcId, preconditionStateVersion: plan.preconditionStateVersion, resultingStateVersion: plan.resultingStateVersion, npcPublicationId: publication.publicationId, commitResultRequestId: commitResult.requestId };
  return { plan, binding, validatedCandidateBinding, commitDelta, npcClaim, npcEvent, publication, commitResult, idempotencyRecord };
}
...
test("ReactionPlan enforces descriptor/segment compatibility and exact policies", () => {
  ...
  assert.throws(() => validateNpcReactionPlan({ ...plan, successfulAttemptId: undefined }));
  assert.throws(() => validateNpcReactionPlan({ ...plan, preconditionStateVersion: Number.MAX_SAFE_INTEGER }));
  assert.throws(() => validateNpcReactionPlan({ ...plan, resultingStateVersion: 3 }), (error) => error.code === "state_version_mismatch");
  assert.throws(() => validateNpcReactionPlan({ ...plan, requestFingerprint: fingerprint }));
  assert.throws(() => validateNpcReactionPlan({ ...plan, turnOrder: 1 }));
  ...
  for (const [descriptorType, type, field] of [["result_claim", "canonical_claim", "claimId"], ["vote_declaration", "canonical_vote", "voteEventId"], ["suspicion", "canonical_suspicion", "suspicionEventId"]]) { const descriptor = descriptorType === "result_claim" ? { descriptorId: "desc-1", descriptorType, targetId: "npc-2", result: "werewolf" } : { descriptorId: "desc-1", descriptorType, targetId: "npc-2" }; const policies = { ...plan.policies, allowClaims: descriptorType === "result_claim", allowVoteDeclaration: descriptorType === "vote_declaration", allowSuspicionUpdate: descriptorType === "suspicion", allowMemoryUpdate: false }; assert.doesNotThrow(() => validateNpcReactionPlan({ ...plan, intendedSpeechActs: [descriptor], policies, canonicalSegments: [{ segmentId: "seg-1", descriptorId: "desc-1", type, [field]: "object-1" }] })); }
});
...
test("canonical ReactionPlan segments require matching Claim provenance", () => {
  const plan = canonicalPlan(), npcSource = { sourceType: "npc_reaction", reactionPlanId: "plan-1", descriptorId: "desc-1", originatingInputRecordId: "input-1", reactionCommitRequestId: "reaction-request-1" }, npcClaim = { ...claim, claimId: "claim-2", actorId: "npc-1", source: npcSource, createdStateVersion: 2, idempotencyKey: npcClaimIdempotencyKey({ reactionCommitRequestId: "reaction-request-1", reactionPlanId: "plan-1", descriptorId: "desc-1", actorId: "npc-1", claimKind: "role_claim" }) };
  assert.doesNotThrow(() => validatePreparedConversationGraph({ inputRecords: [input], reactionPlans: [plan], claims: [npcClaim] }));
  assert.throws(() => validatePreparedConversationGraph({ inputRecords: [input], reactionPlans: [plan], claims: [{ ...npcClaim, source: { ...npcSource, descriptorId: "other" } }] }));
  assert.throws(() => validatePreparedConversationGraph({ inputRecords: [input], reactionPlans: [plan], claims: [] }));
});
...
test("NpcReactionPlan version and correlation derive from originating input", () => {
  const plan = controlledPlan(); assert.doesNotThrow(() => validatePreparedConversationGraph({ inputRecords: [input], reactionPlans: [plan] }));
  assert.throws(() => validatePreparedConversationGraph({ inputRecords: [input], reactionPlans: [{ ...plan, resultingStateVersion: 3 }] }), (error) => error.code === "state_version_mismatch");
  assert.throws(() => validatePreparedConversationGraph({ inputRecords: [input], reactionPlans: [{ ...plan, correlationId: "other" }] }));
});

test("ReactionPlan reference contexts separate pre-commit and committed owners", () => {
  const fixture = npcReferenceFixtures(), preCommit = { schemaVersion: 1, contextType: "pre_commit", preparationBinding: fixture.binding, commitDelta: fixture.commitDelta, validatedCandidateBinding: fixture.validatedCandidateBinding }, committed = { schemaVersion: 1, contextType: "committed_graph", reactionPlan: fixture.plan, idempotencyRecord: fixture.idempotencyRecord, commitResult: fixture.commitResult, publication: fixture.publication, claims: [fixture.npcClaim], events: [fixture.npcEvent], segments: fixture.plan.canonicalSegments };
  assert.equal(validateReactionPlanReferences(fixture.plan, preCommit), true);
  assert.equal(validateReactionPlanReferences(fixture.plan, committed), true);
  assert.throws(() => validateReactionPlanReferences(fixture.plan, { ...preCommit, unknown: true }), (error) => error.code === "invalid_reference_context");
  assert.throws(() => validateReactionPlanReferences(fixture.plan, { ...preCommit, preparationBinding: { ...fixture.binding, resultingStateVersion: 2 } }), (error) => error.code === "invalid_reference_context");
  assert.throws(() => validateReactionPlanReferences(fixture.plan, { ...preCommit, idempotencyRecord: fixture.idempotencyRecord }), (error) => error.code === "invalid_reference_context");
  assert.throws(() => validateReactionPlanReferences(fixture.plan, { ...preCommit, validatedCandidateBinding: { ...fixture.validatedCandidateBinding, reactionAttemptId: "attempt-2" } }), (error) => error.code === "reference_failure");
  assert.throws(() => validateReactionPlanReferences(fixture.plan, { ...preCommit, validatedCandidateBinding: { ...fixture.validatedCandidateBinding, requestFingerprint: "d".repeat(64) } }), (error) => error.code === "reference_failure");
  assert.throws(() => validateReactionPlanReferences(fixture.plan, { ...preCommit, validatedCandidateBinding: { ...fixture.validatedCandidateBinding, turnOrder: 2 } }), (error) => error.code === "reference_failure");
  assert.throws(() => validateReactionPlanReferences(fixture.plan, { ...committed, idempotencyRecord: { ...fixture.idempotencyRecord, successfulAttemptId: "attempt-2" } }), (error) => error.code === "corrupt_authoritative_graph");
  assert.throws(() => validateReactionPlanReferences(fixture.plan, { ...committed, idempotencyRecord: { ...fixture.idempotencyRecord, requestFingerprint: "d".repeat(64) } }), (error) => error.code === "corrupt_authoritative_graph");
});

test("committed canonical NPC graphs require complete idempotency ownership without coordinator state", () => {
  const fixture = npcReferenceFixtures(), informationAct = { ...actBase, type: "accepted_information_request", topic: "rules" }, displayPlan = { schemaVersion: 1, displayPlanId: "display-info", inputRecordId: "input-1", turnId: "turn-1", stateVersion: 1, segments: [{ segmentId: "raw-info", type: "raw_input", inputRecordId: "input-1", sourceSpan: { start: 0, end: 16 } }] }, playerPublication = { schemaVersion: 1, recordType: "player_utterance_published", publicationId: "pub-info", requestId: "request-1", correlationId: "corr-1", turnId: "turn-1", gameStateVersion: 1, occurredPhase: "day_discussion", actorId: "player", inputRecordId: "input-1", displayPlanId: "display-info", idempotencyKey: "idem-info", publicationSlotOrder: 0, recordAppendOrder: 0 }, playerResult = { schemaVersion: 1, requestId: "request-1", correlationId: "corr-1", requestFingerprint: fingerprint, commitType: "player_conversation", preconditionStateVersion: 0, resultingStateVersion: 1, inputRecordId: "input-1", displayPlanId: "display-info", playerPublicationId: "pub-info", createdEventIds: [], createdClaimIds: [], createdAtOrder: 0 }, graph = { inputRecords: [input], acceptedSpeechActs: [informationAct], displayPlans: [displayPlan], reactionPlans: [fixture.plan], claims: [fixture.npcClaim], events: [fixture.npcEvent], publications: [playerPublication, fixture.publication], commitResults: [playerResult, fixture.commitResult], npcReactionCommitIdempotencyRecords: [fixture.idempotencyRecord] };
  assert.equal(validateCommittedConversationGraph(graph), true);
  assert.equal(validateCommittedConversationGraph({ ...graph, coordinatorRoot: undefined, tombstones: [] }), true);
  assert.throws(() => validateCommittedConversationGraph({ ...graph, npcReactionCommitIdempotencyRecords: [] }), (error) => error.code === "incomplete_authoritative_graph");
  assert.throws(() => validateCommittedConversationGraph({ ...graph, npcReactionCommitIdempotencyRecords: [fixture.idempotencyRecord, structuredClone(fixture.idempotencyRecord)] }), (error) => error.code === "corrupt_authoritative_graph");
  const alias = { ...fixture.idempotencyRecord, gameSessionId: "session-1", reactionPlanId: "plan-2", requestId: "reaction-request-2", commitResultRequestId: "reaction-request-2" };
  assert.throws(() => validateCommittedConversationGraph({ ...graph, npcReactionCommitIdempotencyRecords: [fixture.idempotencyRecord, alias] }), (error) => error.code === "authoritative_identity_conflict");
});
...
function interpreterRequestFixture() { return { schemaVersion: 1, requestId: "interpreter-request-1", correlationId: "corr-1", inputRecordId: "input-1", turnId: "turn-1", preconditionStateVersion: 0, preconditionPhase: "day_discussion", locale: "ja-JP", rawText: "x", playerContext: { playerId: "player", publicStatus: "alive" }, publicRoster, allowedCandidateTypes: ["non_game_statement"], publicContext: { publicEvents: [publicEventProjections[0]], publicClaims: [], publicVotes: [], executions: [], attackDeaths: [] }, limits: { maxAlternatives: 3, maxActsPerAlternative: 4, maxNestingDepth: 8 } }; }
