...
Structured lifecycle／fallback correctionは、approved head `b9670de1a8c0740321f02dbdcd0861f8ec4a48ec`をsecond parentとする通常の2-parent merge commit `fa7757f72cf4b95b2f52c1f94c0fc6ee2f12b98b`で取り込み済みです。Replacement F-06 acceptanceはこのbaselineからfreshに実行し、新規29/29、focused 511/511、全体760/760とsampleがPASSしています。詳細は[NPC Structured Reaction Acceptance Matrix](docs/npc-structured-reaction-acceptance-matrix.md)を参照してください。production runtime差分は0で、independent reviewとmergeは未完了です。E4 manual real-browser、E5 real OpenAI candidate route、flag-on判断は別工程であり、flagのdefaultは`false`です。旧2件の未完了F-06 goal-management recordはユーザーにより手動削除済みで、再開・完了扱いにはしていません。
