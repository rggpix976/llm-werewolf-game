...
Replacement F-06 acceptanceは、approved head `ed613c7aeb5ce420a73ce045547259129dea9049`をsecond parentとする通常の2-parent merge commit `a5295e2606b55917cae20d937024aebb0d64a4ae`（PR #66）で取り込み済みです。このbaselineからE4 manual real-browserを開始し、E4-001〜005、E4-006 source drain、E4-007 source abandonはPASSしましたが、fresh disabled Browserでlegacy APIが1回2xx成功してEngine logにもNPC responseが存在するのに、最終DOM reconciliation後のlegacy NPC回答が消えるHIGH defectを確認して停止しました。E4は未完了のBLOCKED outcomeであり、E4-008／009は`NOT_RUN`です。現在のDraft runtime correctionは`public/browserApp.mjs`のreconciliationだけを修正し、`playerFacingLog`のIDなしlegacy entryもvisible modelとして保持しながら、canonical NPC anchor countはpublication-backed Player nodeだけで進めます。Engine、API、Provider、Route、Commit、Delivery、flag defaultは変更しません。詳細は[Browser Legacy Display Reconciliation Decision](docs/browser-legacy-display-reconciliation-decision.md)と[NPC Structured Reaction Acceptance Matrix](docs/npc-structured-reaction-acceptance-matrix.md)を参照してください。Correctionの独立レビュー／merge、新baselineからのreplacement E4、E5 real OpenAI candidate routeは未完了で、flagのdefaultは引き続き`false`です。過去の未完了Goal recordの手動削除はcompletionまたはresumeを意味しません。
