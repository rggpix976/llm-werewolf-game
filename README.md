...
Structured lifecycle／fallback correctionは、approved head `b9670de1a8c0740321f02dbdcd0861f8ec4a48ec`をsecond parentとする通常の2-parent merge commit `fa7757f72cf4b95b2f52c1f94c0fc6ee2f12b98b`で取り込み済みです。Replacement F-06 acceptanceはこのbaselineからfreshに実行しました。PR #66のreviewed HEAD `21a166d144d389a389b5193b13b6b3c10e3f7b12`に対する`CHANGES_REQUESTED`後、production runtimeを変更せず、actual `ack_only` composition、CLI `repeat_sink`／`ack_only`、candidate HTTP／disconnect、Browser late-result、privacy／identity evidenceを補強しました。dead NPCへのclosed rejectionは既存contractどおりnew logical turn identityを保持し、`stateVersion`とPlayer／NPC conversation、Delivery、legacy effectを変更しません。Review-repair後は新規32/32、focused 600/600、全体763/763とsampleがPASSしています。詳細は[NPC Structured Reaction Acceptance Matrix](docs/npc-structured-reaction-acceptance-matrix.md)を参照してください。production runtime defectの確認は0で、exact repair HEADのindependent rereviewとmergeは未完了です。E4 manual real-browser、E5 real OpenAI candidate route、flag-on判断は別工程であり、flagのdefaultは`false`です。旧2件の未完了F-06 goal-management recordはユーザーにより手動削除済みで、再開・完了扱いにはしていません。
