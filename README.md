...
Phase 6 hardeningでは、Finding F-01のcandidate cost controlに加え、F-02のPlayer-before-NPC delivery sequencingとF-03のBrowser New Game cleanupが通常のmerge commit `a039bd1f19e83bbe8ba65573071c0dc64f3b15c9`で取り込み済みです。Structured Routeとauthoritative NPC Commitをengine内で完了した後も、Browser／CLIがPlayer表示を`acknowledged`または`evidence_recorded`へ完了するまでNPC Deliveryを開始しません。Player表示またはNPC Deliveryがretry可能な場合は同じfrozen action／engine-private handoffを保持し、Browserでは同じAsk controlの`Retry Display`、CLIでは明示的な`retry` commandでのみ再開します。`repeat_sink`はsinkだけを、`ack_only`はacknowledgementだけを再試行し、Provider、Validation、Preparation、Commit、Player action dispatchを再実行しません。Browser New Gameは旧generationを先に無効化し、旧sessionのNPC DOM nodeをbookkeeping reset前に除去してlate callbackと再挿入を遮断します。F-04 observabilityは通常のmerge commit `759b645591d479979d3ad50cab420adb1f8879ab`、F-05 rollback runbookは`35ddce7ffba21253b89c17ece92a98316cfaa7e4`、post-winner authoritative no-op correctionは`1b2db28fad116d3262409e606ba2227de6496f80`で取り込み済みです。

Structured lifecycle／fallback correctionは、approved head `b9670de1a8c0740321f02dbdcd0861f8ec4a48ec`をsecond parentとする通常の2-parent merge commit `fa7757f72cf4b95b2f52c1f94c0fc6ee2f12b98b`で取り込み済みです。Replacement F-06 acceptanceはこのbaselineからfreshに実行しました。PR #66のreviewed HEAD `21a166d144d389a389b5193b13b6b3c10e3f7b12`と最初のrepair HEAD `9d1e68d94680d8c12ca1732e1190733e298b78fd`はいずれも独立レビューで`CHANGES_REQUESTED`となり、production runtime defectの確認は0でした。現在のtest／docs-only follow-upはauthoritative stateへの直接代入を除去し、production-generatedなfrozen `request.knownInformation.actorPrivate`と注入failure sourceの存在を先に証明してから、Engine／Browser／CLI／Server／typed lifecycleの対応public surfaceで非露出を検証します。Identity evidenceは`before -> intermediate authoritative commit -> after Delivery`を分離し、exact registry additions、既存prefix不変、全primary IDのuniqueness、Player／NPC／Route／Deliveryのcross-object binding、Deliveryによるcanonical mutation 0を固定します。dead NPCへのclosed rejectionは既存contractどおりnew logical turn identityを保持し、`stateVersion`とPlayer／NPC conversation、Delivery、legacy effectを変更しません。Follow-up後は新規32/32、focused 600/600、全体763/763とsampleがPASSしています。詳細は[NPC Structured Reaction Acceptance Matrix](docs/npc-structured-reaction-acceptance-matrix.md)を参照してください。正式状態は`F06_REPLACEMENT_ACCEPTANCE_CHANGES_REQUESTED`であり、latest follow-up HEADのindependent rereviewとmergeは未完了です。E4 manual real-browserとE5 real OpenAI candidate routeは`NOT_RUN`、flag-on判断は別工程であり、flagのdefaultは`false`です。旧2件の未完了F-06 goal-management recordはユーザーにより手動削除済みで、再開・完了扱いにはしていません。

F-04のNPC production observabilityは、Structured Route、Delivery Controller、Delivery Orchestratorの既存redacted eventだけをsession-localなin-memory ledgerへ統合します。Browser／CLIともcapacityは200件で、古いrecordからevictされます。BrowserではDeveloper Modeの最新100件、CLIでは`dev`の最新20件と`--show-dev`の未表示差分だけを表示します。Player-facing UI／通常CLI出力には表示せず、New Game／process終了でsession ledgerを破棄します。永続化、remote telemetry、network送信、raw prompt／response、Known Information、role／team、provider exceptionは保存しません。Structured Route flagがOFFのsessionと外部注入CLI gameではledgerを作らず、診断はunavailableです。F-04は通常のmerge commit `759b645591d479979d3ad50cab420adb1f8879ab`で取り込み済みです。

Phase 6 Structured Routeの停止手順は[NPC Structured Reaction Rollback Runbook](docs/npc-structured-reaction-rollback-runbook.md)を参照してください。flagのhot toggleはなく、Server／CLI process restartとBrowserのfull reloadまたはtab reopenが必要です。Browser New Gameだけではcached runtime configが更新されないため不十分です。Structured Routeだけを止めるObjective Aと、そのprocessのOpenAI trafficも止めるObjective Bは別手順です。emergency rollbackではold in-memory sessionをabandonし、同じreactionのlegacy resendやcross-process recoveryを行いません。`NPC_STRUCTURED_REACTION_MODE`はdefault-offを維持し、Slice 6 candidate routeのbillable live smokeは未実施です。
