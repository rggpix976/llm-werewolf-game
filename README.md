...
独立したStructured lifecycle／fallback correctionは、successful NPC commitでlive phaseを同じatomic replacement内で`player_question`から`day_discussion`へ閉じ、Player commit後かつNPC commit前のterminal failureではengine-owned CAS settlementだけで同じlive phaseを閉じます。Structured mode ONのaccepted `ask_npc`はInterpreterのnonvalidated／malformed／unknown outcomeでもlegacy Provider／displayへfall throughしません。現在はDraft correction branch上の実装であり、independent reviewとmergeは未完了です。Replacement F-06 acceptance自体は未実行・未完了で、旧BLOCKED goal-management recordはユーザーにより手動削除済みです。correctionのmerge後に新baselineから別Goalとして再作成・再実行する必要があります。flag-on判断とSlice 6 candidate routeのbillable live smokeは未実施で、flagのdefaultは`false`です。
