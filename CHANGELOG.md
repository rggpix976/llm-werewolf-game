...
- Added the default-off Migration Phase 5 Draft structured publication consumer for browser and CLI display/history, with stored-plan resolution, canonical claim/vote/suspicion rendering, session-local publication deduplication, and exact-replay no-redisplay.
- The Draft Phase 5 flag stops the legacy player display while retaining legacy storage and the unchanged NPC legacy path; this behavior remains incomplete pending exact compatibility-mapping consumption and the sink-success/acknowledgement protocol.
- The Phase 4 compatibility-mapping prerequisite is now present on this branch. Phase 5 remains Draft and is not complete until exact mapping consumption and the sink-success/acknowledgement protocol are implemented and reviewed.
