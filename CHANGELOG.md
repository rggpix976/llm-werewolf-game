...
- Added the production-unconnected Phase 6 NPC reaction coordinator control-root foundation. The browser-safe module owns the exact seven-field session root, immutable logical and attempt records, terminal-slot reservation and bounded tombstone registries, retry-policy limits, two-step candidate observation with engine-owned fingerprints, closed identity-conflict terminalization, atomic committed/non-commit cleanup, idempotent already-cleaned results, and reset/destroy invalidation. It does not connect `WerewolfGame`, providers, routes, authoritative commit, publication, delivery, or Renderer behavior.
