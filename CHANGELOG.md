...
- Defined the Phase 6 authoritative NPC reaction commit contract: exact pre-provider replay, a strict reaction idempotency record and uniqueness indexes, replay-before-CAS ordering, complete final-CAS dimensions, copy-on-write graph publication, lifecycle/terminal-slot/tombstone ordering, same-transaction canonical publication, replacement of the provisional legacy `N+1 -> N+2`, and a prohibition on any third transition. Runtime preparation/commit/replay/coordinator/provider/delivery behavior remains unchanged.
