...
- Defined the Phase 6 authoritative NPC reaction preparation contract: the canonical-only plan fields remain mandatory, one synchronous pure preparation API consumes strict validated-candidate/snapshot/allocation/order evidence, all four proposals map deterministically to descriptors/claims/events/segments, initial suspicion/memory/legacy-history/vote/phase deltas are exactly empty, and the prepared delta/result/rejection/invariant boundaries remain nonauthoritative until a separately designed final commit. Runtime preparation, provider routing, commit, and delivery are unchanged.
